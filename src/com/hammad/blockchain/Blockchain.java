...
public class Blockchain {
	...
	public boolean isFristBlockValid() {
	...
	public boolean ifValidNewBlock(Block newBlock, Block previousBlock) {
		if(newBlock != null && previousBlock != null) {
			if(previousBlock.getIndex() + 1 != newBlock.getIndex()) {
				return false;
			}
			
			if(newBlock.getHash() == null || !newBlock.getPreviousHash().equals(previousBlock.getHash())) {
				return false;
			}
	...
	public boolean isBlockChainValid() {
		...
		for(int i=1; i<blocks.size(); i++) {
			...
			if(!ifValidNewBlock(newBlock, previousBlock)) {
				return false;
			}
		}
		return true;
